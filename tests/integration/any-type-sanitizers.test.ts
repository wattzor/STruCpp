 ...
 *
 * AnyType passes a raw pointer (PVALUE) to the caller's storage, so CODESYS
 * only allows variable arguments. The runtime still has to keep that storage
 * alive for the duration of the call; the variable-argument path is exercised
 * here under sanitizers.
 */
